package co.touchlab.squeaky.android;
 ...
 * @author graywatson
 */
public class AndroidLog implements co.touchlab.squeaky.logger.Log {
	...
	public AndroidLog(String className) {
		...
		int maxLevel = 0;
		for (co.touchlab.squeaky.logger.Log.Level level : co.touchlab.squeaky.logger.Log.Level.values()) {
	...
	private int levelToAndroidLevel(co.touchlab.squeaky.logger.Log.Level level) {
		switch (level) {
			case TRACE :
				return Log.VERBOSE;
			case DEBUG :
